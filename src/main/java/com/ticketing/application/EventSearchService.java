...
/**
 * Public search over events. Token-less (guest-callable). Returns only events
...
public class EventSearchService {
    ...
    public List<EventSummaryDTO> searchEvents(SearchEventsRequest req) {
        ...
        List<EventSummaryDTO> hits = eventRepository.findAll().stream()
                .filter(EventSearchService::isBrowsableStatus)
    ...
    /** True if any zone's price falls in [min, max] (either side optional). */
